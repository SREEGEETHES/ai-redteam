...
@st.cache_data(ttl=5)
def api_get_raw(endpoint: str, params: dict = None):
    try:
        response = requests.get(f"{API_BASE_URL}{endpoint}", params=params, timeout=10)
        response.raise_for_status()
        return response
...
def progress_bar(pct: float, color=ACCENT) -> str:
    ...
    return f'<div class="rt-bar"><div style="width:{pct*100:.0f}%;background:{color}"></div></div>'
...
_ID_COLS = {"id", "attack_id", "test_id", "scan_id", "target_id", "finding_id", "check_id", "retest_id", "hash"}
...
def _fmt_cell(col: str, v) -> str:
    ...
    if isinstance(v, (list, tuple, set)):
        return "".join(f'<span class="rt-chip-sm">{esc(i)}</span>' for i in v) or '<span class="rt-dim">—</span>'
...
def render_table(data, cols=None, max_height=420, limit=500):
    ...
    for rec in df.to_dict("records"):
        rows.append("<tr>" + "".join(f"<td>{_fmt_cell(str(c), rec[c])}</td>" for c in df.columns) + "</tr>")
    md(
...
def _style_chart(chart, title=None, height=260):
    if title:
        chart = chart.properties(
            title=alt.TitleParams(title, color=MUTED, fontSize=11, fontWeight=600, anchor="start", offset=10)
        )
    return (
        chart.properties(height=height, width="container", background="transparent",
                         autosize=alt.AutoSizeParams(type="fit", contains="padding"),
                         padding={"left": 6, "right": 26, "top": 16, "bottom": 6})
        .configure_view(stroke=None)
        .configure_axis(
            labelColor=MUTED, titleColor=MUTED, gridColor="#202A36", domainColor=BORDER,
            tickColor=BORDER, labelFontSize=11, titleFontSize=11, labelFont=FONT, titleFont=FONT,
        )
        .configure_legend(labelColor=TEXT, titleColor=MUTED, orient="bottom", direction="horizontal",
                          labelFontSize=12, labelFont=FONT, symbolType="circle")
...
def bar_chart(counts: dict, title: str, colors: dict = None, order=None, horizontal=False, height=240):
    if not counts:
        return
    df = pd.DataFrame({"label": list(counts.keys()), "value": list(counts.values())})
    ...
    if colors:
        color = alt.Color(
            "label:N",
            scale=alt.Scale(domain=list(counts.keys()), range=[colors.get(k, GREY) for k in counts]),
            legend=None,
        )
    else:
    ...
    if horizontal:
        chart = alt.Chart(df).mark_bar(cornerRadiusEnd=4, height=16).encode(
            ...
            color=color, tooltip=["label:N", "value:Q"],
        )
    else:
        chart = alt.Chart(df).mark_bar(cornerRadiusTopLeft=4, cornerRadiusTopRight=4, size=38).encode(
...
def main():
    ...
    with st.sidebar:
        ...
        page = st.radio("Navigation", list(pages.keys()), label_visibility="collapsed", key="nav_page")
        try:
...
def show_overview():
    ...
    kpi_row([
        ...
        ("Findings (FAIL)", len(findings), "Confirmed vulnerabilities", RED if findings else GREEN),
        ("Pass / Fail", f"{pass_c} / {fail_c}", f"Pass rate {rate}", GREEN if fail_c == 0 else AMBER),
    ])
    ...
    if tests:
        ...
        with c2:
            if findings:
                ...
                sev = {k: sev[k] for k in SEV_ORDER if k in sev} | {k: v for k, v in sev.items() if k not in SEV_ORDER}
                bar_chart(sev, "FINDINGS BY SEVERITY", colors=SEV_COLORS, order=SEV_ORDER, height=250)
            else:
                empty_state("No findings recorded — every executed test passed or was inconclusive")
        section("Latest tests")
        render_table(df, ["id", "attack_id", "category", "name", "result", "severity"], max_height=440, limit=20)

    if scans:
        section("Recent scans")
        df = pd.DataFrame(scans[:10])
        render_table(df, ["id", "target_id", "taxonomy_version", "status", "started_at", "completed_at"])
    elif not tests:
...
def show_targets():
    ...
    with tab1:
        targets = api_get("/targets")
        if targets:
            render_table(targets, ["id", "name", "target_type", "base_url", "is_authorized", "created_at"])
            ...
            for t in targets:
                ...
                with col2:
                    clicked = st.button("Run check", key=f"health_{t['id']}", use_container_width=True)
                if clicked:
                    try:
    ...
    with tab3:
        ...
        if targets:
            ...
            if tid:
                ...
                if checks:
                    render_table(
                        [{"check_id": c.get("check_id"), "name": c.get("name"),
                          "passed": c.get("passed"), "reason": c.get("reason")} for c in checks]
                    )
                else:
...
def show_scan():
    ...
    categories = sorted(set(a["category"] for a in attacks)) if attacks else []
    ...
    if "polling_scan_id" in st.session_state:
        ...
        if isinstance(scan, dict) and scan.get("id"):
            ...
            with c1:
                md(kpi("Status", str(scan.get("status", "—")), "", status_color(scan.get("status"))))
            with c2:
                md(kpi("Started", str(scan.get("started_at") or "—").replace("T", " ")[:19], "", BLUE))
            with c3:
                md(kpi("Completed", str(scan.get("completed_at") or "—").replace("T", " ")[:19], "", TEAL))
            ...
            if tests:
                ...
                render_table(
                    tests,
                    ["attack_id", "category", "name", "result", "severity", "reproduction_count", "confidence"],
                )
    ...
    scans = api_get("/scans")
    if scans:
        render_table(scans, ["id", "target_id", "taxonomy_version", "status", "started_at", "completed_at"])
...
def show_findings():
    ...
    render_table(findings, ["id", "attack_id", "category", "title", "severity", "regression_status"])
    ...
    with col_exp1:
        if st.button("📥 Export Findings (JSON)", use_container_width=True):
            ...
            st.download_button("Download JSON", data, file_name=f"findings_{scan_id}.json", mime="application/json")
    with col_exp2:
        if st.button("📥 Export Findings (CSV)", use_container_width=True):
            df = pd.DataFrame(findings)
            csv = df.to_csv(index=False)
            st.download_button("Download CSV", csv, file_name=f"findings_{scan_id}.csv", mime="text/csv")
    with col_exp3:
    ...
    if selected:
        ...
        with col_btn1:
            if st.button("🔁 Retest this finding", key=f"retest_{f['id']}", type="primary"):
                ...
                if res:
                    flash("success", f"Retest {res['retest_id']} → {res['regression_status']} ({res['result']})")
                    st.rerun()
        with col_btn2:
        ...
        with col_btn3:
            ...
            if st.button("📥 Export finding (JSON)", use_container_width=True):
                ...
                st.download_button("Download finding JSON", data, file_name=f"finding_{f['id']}.json", mime="application/json")
...
# ─────────────────────────────────────────────────────────────────────────────
def show_evidence():
    page_header("Evidence", "Request · response · tool calls · retrieval · detectors · reproduction (immutable)")
    ...
    for e in evidences:
        ...
        rows.append({
            ...
            "attack_id": req.get("attack_id") if isinstance(req, dict) and req.get("attack_id") else e.get("test_id"),
            "result": e.get("result"),
    ...
    if selected:
        ...
        with st.expander("Response capture", expanded=True):
            json_block({"http_status": ev.get("http_status"), "headers": ev.get("headers"), "response": ev.get("response")})
        ...
        for f in findings:
            if f.get("test_id") == selected:
                if st.button(f"Compare before / after — finding {f['id']}", key=f"compare_{f['id']}"):
...
def show_owasp_coverage():
    ...
    if scans:
        ...
        kpi_row([
            ...
            ("Categories", len(set(a["category"] for a in attacks)) if attacks else 0, "Covered", TEAL),
        ])
        ...
        with c2:
            if cov_counts:
                bar_chart(cov_counts, "ATTACKS PER OWASP CATEGORY", horizontal=True, order="ascending", height=260)
    if attacks:
        section("Attack catalogue")
        render_table(attacks, ["attack_id", "category", "name", "severity", "target_types"], max_height=460)
        section("Coverage by category")
        cats = sorted(set(a["category"] for a in attacks))
        cols = st.columns(2)
        for i, cat in enumerate(cats):
            c_atks = [a for a in attacks if a["category"] == cat]
            chips = "".join(f'<span class="rt-chip-sm">{esc(a["attack_id"])}</span>' for a in c_atks)
            with cols[i % 2]:
                md(
...
def show_remediation():
    ...
    for f in findings:
        with st.expander(f"{f.get('attack_id')} — {f.get('title')}   [{f.get('severity')}]", expanded=False):
...
def show_retest():
    ...
    st.caption("Real retest: a new scan of the same attack, compared against the original and tracked for regression.")
    scans = api_get("/scans")
    if not scans:
        empty_state("No scans available")
        return
    ...
    with col1:
        if st.button("Retest finding", key=f"do_retest_{fid}", type="primary", use_container_width=True):
...
def show_regression():
    ...
    if not all_findings:
        ...
        return
    render_table(all_findings, ["id", "attack_id", "category", "title", "severity", "regression_status"])
    section("Lifecycle")
    fid = st.selectbox(
    ...
    if fid:
        ...
        if isinstance(lc, dict) and lc:
            kpi_row([
                ("Current status", str(lc.get("current_status", "—")).replace("_", " "), "", status_color(lc.get("current_status"))),
...
def show_checklist():
    ...
    kpi_row([
        ("Overall progress", f"{verified / total * 100:.0f}%" if total else "0%", f"{verified} of {total} tasks verified", GREEN),
...
def show_scan_history():
    ...
    render_table(df, ["id", "target_id", "taxonomy_version", "status", "started_at", "completed_at", "created_at"])
    section("Scan detail")
    ...
    if tests:
        st.markdown("**Tests**")
        render_table(tests, ["attack_id", "category", "name", "result", "severity", "reproduction_count", "confidence"])
    findings = api_get(f"/scans/{sid}/findings")
    if findings:
    ...
    if evidences:
        ...
        json_block(
            [{"test_id": e.get("test_id"), "result": e.get("result"), "detectors": e.get("detectors_triggered")} for e in evidences],
            max_height=300,
        )
        ...
        if st.button("Generate JSON report", key=f"json_{sid}", use_container_width=True):
            ...
            if r:
                ...
                st.download_button("Download JSON", json.dumps(data, indent=2, default=str),
                                   file_name=f"scan_{sid}_report.json", mime="application/json")
    with col2:
        if st.button("Generate Markdown report", key=f"md_{sid}", use_container_width=True):
            ...
            if r:
                st.text(r.text[:3000])
                st.download_button("Download Markdown", r.text, file_name=f"scan_{sid}_report.md", mime="text/markdown")
    with col3:
        if st.button("Generate HTML report", key=f"html_{sid}", use_container_width=True):
            ...
            if r:
                ...
                md(f'<a href="{esc(API_BASE_URL)}/scans/{esc(sid)}/report?format=html" target="_blank">Open report in new tab ↗</a>')
                st.download_button("Download HTML", r.text, file_name=f"scan_{sid}_report.html", mime="text/html")
...
def show_configuration():
    ...
    attacks = api_get("/attacks")
    kpi_row([
        ("Current taxonomy", scans[0]["taxonomy_version"] if scans else "—", "From latest scan", ACCENT),
        ("Total attacks", len(attacks) if attacks else 0, "Definitions loaded", BLUE),
        ("API endpoint", API_BASE_URL.replace("http://", "").replace("https://", ""), "Backend service", TEAL),
    ])
    section("Per-scan budgets")
    b = st.columns(4)
    budgets = [("Max requests", "100"), ("Max tokens", "50,000"), ("Max runtime", "300 s"), ("Max tool calls", "50")]
    for col, (label, value) in zip(b, budgets):
        with col:
            md(kpi(label, value, "per scan", AMBER))
    section("Authorization")
    st.info("Target authorization: localhost by default; an explicit allowlist is required for remote targets "
...
def show_attacks():
    ...
    kpi_row([("Total", len(attacks), "Attack definitions", ACCENT)] +
