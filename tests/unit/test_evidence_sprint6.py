...
from app.database.models import Evidence, Scan, Target, Test
...
def test_response_capture():
    raw = {
        ...
        "tool_calls": [{"tool": "search"}],
        "retrieved_documents": [{"id": "doc-1", "metadata": {"title": "t"}}],
...
def test_tool_call_capture_detailed():
    raw = {
        ...
        "execution_results": [{"status": "deleted"}],
        "side_effects": ["DATABASE DELETED"],
        "headers": {},
    }
...
def test_deterministic_detection():
    ...
    ev1, det1 = build_evidence(
        scan_id=1,
        test_id=1,
        target="http://localhost:8000",
        attack_id="LLM02-SD-001",
        raw=raw,
        detector_ids=["canary_secret_leak"],
...
def test_evidence_hash_and_immutability():
    raw = RawObservation(
        request={"message": "hi"},
        response_raw={"status_code": 200, "response": {"response": "hello"}, "headers": {}},
        baseline=None,
    )
    ev, _ = build_evidence(
        scan_id=1,
...
def test_every_fail_has_reproducible_evidence_via_orchestrator():
    ...
    with patch.object(orchestrator.AdapterRegistry, "create_adapter", lambda *a, **kw: MockVuln()):
        ...
        assert (
            ev.evidence_metadata["reproduction_count"] == 2
            or ev.evidence_metadata["reproduction_count"] == 1
        )
        ...
        rec = EvidenceRecord(
            ...
            detectors_triggered=ev.detectors_triggered,
            expected_behavior=ev.expected_behavior,
            observed_behavior=ev.observed_behavior,
            reproduction_count=ev.evidence_metadata["reproduction_count"],
            confidence=ev.evidence_metadata["confidence"],
            result=t.result,
        )
        summary = evs(rec)
    ...
    class MockVuln:
        ...
        def send_attack(self, payload):
            return {
                ...
                "response": {"response": "CANARY_SECRET_12345"},
                "headers": {},
                "tool_calls": [{"tool": "search", "arguments": {"query": "test"}}],
                "retrieved_documents": [{"id": "doc-1"}],
            }
