...
def test_orchestrator_end_to_end_vulnerable(monkeypatch):
    ...
    monkeypatch.setattr(
        orchestrator.AdapterRegistry, "create_adapter", lambda *a, **kw: MockLLMVuln()
    )

    result_scan = orchestrator.run_scan(scan.id, db)
    assert result_scan.status.value == "COMPLETED"

    from app.database.models import Evidence, Finding, Test
...
def test_orchestrator_end_to_end_secure(monkeypatch):
    ...
    monkeypatch.setattr(
        orchestrator.AdapterRegistry, "create_adapter", lambda *a, **kw: MockLLMSecure()
    )

    result_scan = orchestrator.run_scan(scan.id, db)
    assert result_scan.status.value == "COMPLETED"

    from app.database.models import Finding, Test
...
def test_orchestrator_not_applicable_filter(monkeypatch):
    ...
    scan = Scan(
        target_id=target.id,
        taxonomy_version="owasp-llm-2026",
        configuration={"attack_ids": ["LLM06-AGENT-001"]},
        status="PENDING",
    )
    db.add(scan)
    db.commit()
