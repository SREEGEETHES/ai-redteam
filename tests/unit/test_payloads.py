...
from app.attacks.payloads import PAYLOAD_REGISTRY, generate_payload
...
def test_llm_direct_payload_shape():
    payload, _meta = generate_payload("llm_direct_prompt_injection")
    assert "message" in payload
    assert "Ignore" in payload["message"]


def test_rag_cross_tenant_shape():
    payload, _meta = generate_payload("rag_cross_tenant_retrieval")
    assert "query" in payload
    assert "tenant" in payload


def test_agent_payload_shape():
    payload, _meta = generate_payload("agent_unauthorized_tool")
...
def test_benign_control_payload():
    payload, _meta = generate_payload("benign_control")
