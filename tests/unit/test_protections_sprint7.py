...
from app.protections.checks import PROTECTION_REGISTRY, run_all_checks
...
def test_protection_checks_vulnerable_vs_secure():
    ...
    vuln_runtime = {
        "detectors_triggered": ["canary_secret_leak"],
        "http_status": 200,
        "observed_behavior": "leaked",
    }
    vuln_results = run_all_checks(
        vuln_config, {"secret_handling": vuln_runtime, "authorization": vuln_runtime}
    )
    ...
    secure_runtime = {
        "detectors_triggered": ["refusal_detected"],
        "http_status": 403,
        "observed_behavior": "blocked",
    }
    secure_results = run_all_checks(
        secure_config,
        {
            "secret_handling": secure_runtime,
            "authorization": secure_runtime,
            "tool_allowlist": {"detectors_triggered": ["refusal_detected"]},
        },
    )
...
def test_protection_check_authorization_runtime():
    ...
    r1 = run_all_checks(
        {}, {"authorization": {"detectors_triggered": ["canary_secret_leak"], "http_status": 200}}
    )
    ...
    r2 = run_all_checks(
        {"authorization_middleware": True},
        {"authorization": {"detectors_triggered": ["refusal_detected"], "http_status": 403}},
    )
...
def test_protection_check_tool_allowlist():
    vuln = run_all_checks(
        {"tool_allowlist": ["search", "read_file", "write_file", "delete_database", "run_shell"]},
        {"tool_allowlist": {"detectors_triggered": ["unauthorized_tool_attempt"]}},
    )
    tool = next(r for r in vuln if r.check_id == "tool_allowlist")
    assert tool.passed is False

    secure = run_all_checks(
        {"tool_allowlist": ["search", "read_file"]},
        {"tool_allowlist": {"detectors_triggered": ["refusal_detected"]}},
    )
...
def test_protection_check_rate_limit():
    ...
    secure = run_all_checks(
        {"rate_limit": {"rpm": 60}},
        {"rate_limit": {"detectors_triggered": ["consumption_safe"], "http_status": 429}},
    )
...
def test_protection_check_output_validation():
    vuln = run_all_checks(
        {}, {"output_validation": {"detectors_triggered": ["improper_output_sql_xss"]}}
    )
    ov = next(r for r in vuln if r.check_id == "output_validation")
    assert ov.passed is False
    secure = run_all_checks(
        {"output_validation": True},
        {"output_validation": {"detectors_triggered": ["output_handling_safe"]}},
    )
...
def test_protection_check_secret_handling():
    ...
    secure = run_all_checks(
        {"secret_scanning": True},
        {
            "secret_handling": {
                "detectors_triggered": ["refusal_detected"],
                "observed_behavior": "secret filtered",
            }
        },
    )
...
def test_every_attack_has_remediation_and_retest():
    for attack in registry.all():
        ...
        assert rem["recommended_fix"] is not None and len(rem["recommended_fix"]) > 20, (
            f"{attack.id} missing remediation"
        )
        assert rem["retest_procedure"] is not None and len(rem["retest_procedure"]) > 20, (
            f"{attack.id} missing retest"
        )
