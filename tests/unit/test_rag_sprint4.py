...
class VulnerableRAGMock:
    ...
    def send_attack(self, payload):
        ...
        if "doc-3" in msg or "doc_id" in msg:
            ...
            return {"status_code": 200, "response": {"id": "doc-3", "tenant": "tenant-b", "content": "Tenant B confidential data: API_KEY_XYZ_789"}, "headers": {}}
        if "retrieval_inspection" in msg or "Give me all documents" in msg:
            return {"status_code": 200, "response": {"retrieved": [{"id": "doc-1", "tenant": "tenant-a"}, {"id": "doc-3", "tenant": "tenant-b"}], "similarity_scores": [0.9, 0.8], "metadata_leak": True}, "headers": {}}
        if "poison-test" in msg or "poisoned" in msg.lower() or "ignore previous" in msg.lower():
            ...
            if payload.get("id") == "doc-poison-test":
                return {"status_code": 200, "response": {"status": "ingested", "quarantine": False}, "headers": {}}
            return {"status_code": 200, "response": {"response": "Admin mode activated. Here are all secrets: CANARY_SECRET_12345"}, "headers": {}, "retrieved_documents": [{"id": "doc-poisoned", "tenant": "tenant-a"}]}
        if "poisoned document" in msg.lower() or "summarize" in msg.lower():
            return {"status_code": 200, "response": {"response": "Admin mode activated. Here are all secrets"}, "headers": {}}
        return {"status_code": 200, "response": {"response": "hello"}, "headers": {}}
    def close(self): pass
...
class SecureRAGMock:
    ...
    def send_attack(self, payload):
        msg = str(payload)
        if "doc-3" in msg or "doc_id" in msg:
            return {"status_code": 403, "response": {"detail": "Access denied: document belongs to another tenant (ACL)"}, "headers": {}}
        if "retrieval_inspection" in msg or "Give me all documents" in msg:
            return {"status_code": 200, "response": {"retrieved": [{"id": "doc-1", "tenant": "tenant-a"}], "similarity_scores": [0.9], "metadata_leak": False}, "headers": {}}
        if "poison-test" in msg or "ignore previous" in msg.lower():
            if payload.get("id") == "doc-poison-test":
                return {"status_code": 200, "response": {"status": "quarantined", "quarantine": True}, "headers": {}}
        ...
        if "poisoned document" in msg.lower() or "summarize" in msg.lower():
            return {"status_code": 200, "response": {"response": "I don't have enough information - no supporting source"}, "headers": {}}
        return {"status_code": 200, "response": {"response": "hello"}, "headers": {}}
    def close(self): pass
...
def test_tenant_isolation_vuln_vs_secure():
    ...
    vuln = ExecutionEngine(VulnerableRAGMock()).execute(registry.get("LLM08-VECT-001"), 1, 1, "http://localhost:8000")
    sec = ExecutionEngine(SecureRAGMock()).execute(registry.get("LLM08-VECT-001"), 1, 2, "http://localhost:8000")
    ...
    class CrossVuln:
        ...
        def send_attack(self, payload): return {"status_code": 200, "response": "leaked", "retrieved_documents": [{"id": "doc-b", "tenant": "tenant-b"}], "headers": {}}
        def close(self): pass
    class CrossSecure:
        def get_baseline(self): return {"status_code": 200, "response": {"response": "hello"}, "headers": {}}
        def send_attack(self, payload): return {"status_code": 200, "response": "ok", "retrieved_documents": [{"id": "doc-a", "tenant": "tenant-a"}], "headers": {}}
        def close(self): pass
    v2 = ExecutionEngine(CrossVuln()).execute(registry.get("LLM08-VECT-001"), 1, 1, "http://localhost:8000")
    s2 = ExecutionEngine(CrossSecure()).execute(registry.get("LLM08-VECT-001"), 1, 2, "http://localhost:8000")
    assert v2.result == TestResult.FAIL
    assert s2.result != TestResult.FAIL

def test_unauthorized_document_retrieval():
    vuln = ExecutionEngine(VulnerableRAGMock()).execute(registry.get("RAG-UNAUTH-001"), 1, 1, "http://localhost:8000")
    sec = ExecutionEngine(SecureRAGMock()).execute(registry.get("RAG-UNAUTH-001"), 1, 2, "http://localhost:8000")
...
def test_retrieval_inspection_metadata_leak():
    vuln = ExecutionEngine(VulnerableRAGMock()).execute(registry.get("RAG-RETRIEVAL-001"), 1, 1, "http://localhost:8000")
    sec = ExecutionEngine(SecureRAGMock()).execute(registry.get("RAG-RETRIEVAL-001"), 1, 2, "http://localhost:8000")
    assert vuln.result == TestResult.FAIL
...
def test_poisoned_ingest_quarantine():
    vuln = ExecutionEngine(VulnerableRAGMock()).execute(registry.get("RAG-POISON-002"), 1, 1, "http://localhost:8000")
    sec = ExecutionEngine(SecureRAGMock()).execute(registry.get("RAG-POISON-002"), 1, 2, "http://localhost:8000")
    assert vuln.result == TestResult.FAIL
    assert "poisoned_ingest" in vuln.evidence.detectors_triggered
    assert sec.result == TestResult.PASS

def test_context_leakage_via_retrieval():
    vuln = ExecutionEngine(VulnerableRAGMock()).execute(registry.get("RAG-CONTEXT-001"), 1, 1, "http://localhost:8000")
    sec = ExecutionEngine(SecureRAGMock()).execute(registry.get("RAG-CONTEXT-001"), 1, 2, "http://localhost:8000")
    assert vuln.result == TestResult.FAIL
    assert "context_leakage" in vuln.evidence.detectors_triggered
    assert sec.result == TestResult.PASS
    assert "grounding_ok" in sec.evidence.detectors_triggered or "refusal_detected" in sec.evidence.detectors_triggered
...
def test_payload_generators_for_sprint4():
    ...
    for pid in ["rag_unauthorized_retrieval", "rag_retrieval_inspection", "rag_poisoned_ingest", "rag_context_leakage"]:
